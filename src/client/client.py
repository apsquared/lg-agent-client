...
class AgentClient:
    ...
    def _parse_stream_line(self, line: str) -> Any | str | None:
    ...
    def stream(
        ...
        For state-based agents, intermediate states are yielded as dictionaries.
        ...
        if not self.agent:
            raise AgentClientError("No agent selected. Use update_agent() to select an agent.")
        request = StreamInput(message=message, state=state, stream_tokens=stream_tokens)
        if thread_id:
            request.thread_id = thread_id
        if model:
            request.model = model
            ...
            with httpx.stream(
                ...
                for line in response.iter_lines():
                    if line.strip():
                        ...
                        if parsed is None:
                            break
                        if isinstance(parsed, (ChatMessage, str)):
                            yield parsed
                        else:
                            # Handle state updates from langgraph agents
                            yield parsed
        except httpx.HTTPError as e:
            raise AgentClientError(f"Error: {e}")
